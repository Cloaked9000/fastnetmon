...
void recalculate_speed() {
    ...
    for(std::map<uint32_t, std::pair<uint32_t, uint8_t> >::iterator iter = bans.begin(); iter != bans.end(); iter++)
    {
        ...
        {
            logger << log4cpp::Priority::INFO << "Would ban: " << convert_ip_as_uint_to_string(iter->first);
        }
        else
        {
          //  logger << log4cpp::Priority::INFO << "Would NOT ban: " << convert_ip_as_uint_to_string(iter->first);
        }
    }
...
void execute_ip_warn(uint32_t client_ip, map_element average_speed_element, std::string flow_attack_details, subnet_t customer_subnet, uint32_t current_warn_interval) {
    ...
    {
        //Global settings have disabled warns?
        if(!global_ban_settings.enable_warn) {
            logger << log4cpp::Priority::INFO << convert_ip_as_uint_to_string(client_ip) << " Will not be warned as global warns are disabled completely";
        }
        else {
